...
// ─── Zod Schema (Updated to include companyName) ──────────────────────────────
const jobSchema = z
  .object({
    companyName: z.string().min(2, "Company name must be at least 2 characters"), // Added this line
...
// ─── Main Page ─────────────────────────────────────────────────────────────────
export default function PostJobPage({Companyinfo}) {
    const Datas = Companyinfo[0]
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const JobsData = {
        ...
        status: "active",
        companyId: User || "",
        Companyname: data.companyName, // Mapping from the form input
        postedAt: new Date().toISOString(),
        visibility: "public",
      };

      console.log("Submitting JobsData: ", JobsData);

      // Uncomment and use server action when ready:
      const Data = await PostJob(JobsData);
      
      if (Data?.result?.insertedId) {
        router.push('/Dashboard/recruiter');
      } else {
        // Fallback layout interaction if not redirecting immediately
        setSubmitted(true);
      }
    } catch (error) {
        ...
        {Datas.status !== 'Approved' ? (
          ...
          <div className="mb-6 flex items-center gap-4 rounded-2xl border border-emerald-800/50 bg-emerald-950/30 px-5 py-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-slate-100 truncate">{Datas.name}</p>
              <p className="text-xs text-slate-400 truncate">
                {COMPANY.industry} · {Datas.location}
              </p>
            </div>
            <div className="flex items-center gap-1.5 rounded-full border border-emerald-700/50 bg-emerald-900/40 px-3 py-1">
              <CheckCircle2 size={11} className="text-emerald-400" />
              <span className="text-xs font-semibold text-emerald-400">{Datas.status}</span>
            </div>
          </div>
        )}
        {
